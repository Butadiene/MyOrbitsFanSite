...
これらは NASA / SSC 素材ではなく、**fy_fp7 さん提供のキャラクターイラスト**を使用しています（パブリックドメインではありません）。

| ファイル | 用途 | 提供元 | 取扱い |
|---|---|---|---|
| `mini-earth-char.png` | 元イラスト（earth キャラ） | © fy_fp7 | 原作者さんのご厚意により当サイト限定で掲載。転載・再配布・改変はご遠慮ください。 |
| `apple-touch-icon.png` | ファビコン／ホーム画面アイコン | © fy_fp7（加工：当サイト） | 上記イラストを白背景に配置したもの。**fy_fp7 さんのイラストを含む**ため同条件。 |
